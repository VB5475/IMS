...
import { AlertCircle, Plus, Trash2, Package, Printer, Save, Filter as FilterIcon } from "lucide-react";
...
import { useAssetsRevaluation } from "../../hooks/useAssetsRevaluation";
...
function resolveEditLoadParams(recordId, listRecord) {
  ...
  return {
    ...
    yearId: listRecord?.yearid ?? listRecord?.YearID ?? session.yearId ?? ARV_CONFIG.CONFIG_YEAR_ID,
...
function mapHeaderValuesToFilterValues(headerValues) {
  ...
  return {
    ...
    frmtype: str(headerValues.frmtype ?? ARV_CONFIG.FRM_TYPE),
    issuetypeid: str(headerValues.issuetypeid ?? ARV_CONFIG.ISSUE_TYPE_ID),
  };
}
...
export default function AssetsRevaluationForm() {
  ...
  const {
    ...
    divisionOptions, configOptions, fetchConfigOptions,
    ...
    clearSaveError,
  } = useAssetsRevaluation(API_BASE_URL);
  ...
  const headerValuesRef = useRef(applyArvHardcodedHeaderValues({
    trancode: "",
  ...
  const filterInitialValues = useMemo(() => {
    ...
    return {
      trandate: getTodayDateInputValue(),
      issuedate: getTodayDateInputValue(),
      frmtype: String(ARV_CONFIG.FRM_TYPE),
      issuetypeid: String(ARV_CONFIG.ISSUE_TYPE_ID),
    };
  }, [loadedFilterValues]);
  ...
  const cascadeResets = useMemo(() => buildArvCascadeResets(headerColumns), [headerColumns]);
  ...
  const loadEditRecord = useCallback(async () => {
    ...
    try {
      ...
      if (!master || !headerValues) throw new Error("Assets Revaluation record not found.");

      headerValuesRef.current = applyArvHardcodedHeaderValues({
      ...
      console.error("[ARV] Edit record load failed:", err);
      setRecordLoadError(err?.message || "Failed to load Assets Revaluation record.");
    } finally {
  ...
  const dropdownSources = useMemo(() => ({
    ...
    frmtype: ARV_FRM_TYPE_OPTIONS,
  }), [divisionOptions, configOptions]);
  ...
  }, [requestGridClear, fetchConfigOptions]);
  ...
  const fetchItemMainGroupOptions = useCallback(async ({ divisionId, configId }) => {
    try {
      ...
      const res = await getLive(ENDPOINTS.FN_FETCH_DATA, {
        ...
        ObjName: ARV_CONFIG.SP_ITEM_MAIN_GROUP,
        JSon: JSON.stringify([{
          ...
          prmfrmtype: ARV_CONFIG.FORM_TAG,
        }]),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      ...
      console.warn("[ARV] Item Main Group fetch failed:", err);
  ...
  const fetchItemSubMainGroupOptions = useCallback(async ({ divisionId, configId, mainGroupId }) => {
    ...
    try {
      ...
      const res = await getLive(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: OBJ_TYPE.FUNCTION,
        ObjName: ARV_CONFIG.SP_ITEM_SUB_MAIN_GROUP,
        JSon: JSON.stringify([{
          prmcompanyid: session.companyId,
          prmdivisionid: Number(divisionId) || 0,
          prmyearid: session.yearId,
      ...
      console.warn("[ARV] Item Sub Main Group fetch failed:", err);
      setItemSubMainGroupOptions([]);
      return [];
    }
  }, [getLive]);
  ...
  const handleSelectItem = useCallback(async () => {
    ...
    try {
      const rbRes = await getLive(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: OBJ_TYPE.FUNCTION,
        ObjName: ARV_CONFIG.SP_RB_META,
        JSon: JSON.stringify([{ prmRBCode: ARV_CONFIG.RB_ITEM_PICKER }]),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      const rbRow = rbRes?.[0];
      ...
      console.error("[ARV] Item picker fetch failed:", err);
  ...
  const handleApplyItemFilter = useCallback(async () => {
    ...
    try {
      const rowRes = await getLive(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: OBJ_TYPE.FUNCTION,
        ObjName: ARV_CONFIG.SP_ITEM_PICKER,
        JSon: JSON.stringify([{
          ...buildArvItemPickerJsonPayload(headerValues),
      ...
      console.error("[ARV] Item filter fetch failed:", err);
      setItemModalError(err?.message || "Failed to fetch items.");
    } finally {
  ...
  const handleSave = useCallback(async () => {
    ...
    setIsSaving(true);
    try {
      const result = await postSave(ARV_CONFIG.SAVE_ENDPOINT, payload);
      ...
      console.error("[ARV Save] Failed:", err);
  ...
  const handleDiscardConfirm = useCallback(() => {
    ...
    localStorage.removeItem(ARV_CONFIG.STORAGE_HEADER_META);
    localStorage.removeItem(ARV_CONFIG.STORAGE_ENTRY_META);
    headerValuesRef.current = applyArvHardcodedHeaderValues({
      trancode: "",
      trandate: getTodayDateInputValue(),
      issuedate: getTodayDateInputValue(),
  ...
  return (
    <div className="workspace-page workspace-page--fill arv-page">
      ...
      <section className="workspace-page__filters">
        {combinedError ? (
          ...
          <EnterpriseFilterPanel
            ...
            title="Assets Revaluation Detail"
      ...
      <section className="arv-grid-section">
        <EntryGrid
          ref={itemGridRef}
          ...
          headerControls={
            <>
              ...
              <button
                ...
                disabled={!isEditMode}
                title="Pick assets for revaluation"
              >
