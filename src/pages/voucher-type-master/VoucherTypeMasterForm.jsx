...
import { Ticket, Save, Pencil, AlertCircle } from "lucide-react";
...
import { VTM_CONFIG, MODAL_TITLE_ADD, MODAL_TITLE_EDIT, MODAL_SUBTITLE } from "./constants";
import "./VoucherTypeMasterPage.css";

// Live RB colnames for the Module/Levy Formula dropdowns are unconfirmed
// beyond the MRD's own Field ID column — try the likely candidates so
// whichever one the live RB actually uses still resolves (same defensive
// pattern as State Master's COUNTRY_KEYS/STATE_TYPE_KEYS).
const MODULE_KEYS = ["moduleid"];
const LEVY_FORMULA_KEYS = ["levyid"];
...
export default function VoucherTypeMasterForm({
  ...
  function resolveOptions(fieldColname) {
    const lowered = colKey(fieldColname);
    if (MODULE_KEYS.includes(lowered)) return moduleOptions;
    if (LEVY_FORMULA_KEYS.includes(lowered)) return levyFormulaOptions;
    return [];
  }
  ...
  function buildControl(field) {
    ...
    // colctrltype 3/11 — Checkbox (Is Conversion)
    if (isMasterCheckboxField(field)) {
      return (
        <input
          type="checkbox"
          className="vtm-form-checkbox"
    ...
    // colctrltype 4 — Dropdown (Module, Levy Formula)
    if (Number(field.colctrltype) === 4) {
      ...
      return (
        <SearchSelect
          ...
          placeholder="Select..."
          disabled={locked}
          className={error ? "vtm-form-dropdown--error" : undefined}
        />
      );
    }
    ...
    return (
      <input
        className={`vtm-form-input${error ? " vtm-form-input--error" : ""}`}
        type="text"
  ...
  function renderControl(field) {
    ...
    return (
      <>
        {control}
        <div className="vtm-form-field-error">{error}</div>
      </>
    );
  }
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const result = await post(VTM_CONFIG.SAVE_ENDPOINT, payload);
      ...
      console.error("[VTM Save] Failed:", err);
        ...
        <>
          ...
          <div className="vtm-form">
            {visibleFields.map((field) => (
              <div key={field.colname} className="vtm-form-row">
                <span className={`vtm-form-label${field.ismandatory ? " vtm-form-label--required" : ""}`}>
                  {getLabel(field)}
                </span>
                <div className="vtm-form-control">
                  {renderControl(field)}
                </div>
              </div>
            ))}
