...
export default function AssetsWriteOffForm() {
  ...
  }, [headerColumns, allColumns, columns, isEditRoute, notify, postSave, resetFormToInitialState, flushPendingCellEvents]);

  const handleSaveAndPrint = useCallback(async () => {
    const saved = await handleSave({ skipPostSave: true });
    if (!saved) return;
    // AssetsWriteOff.rpt has no per-record filter (see constants.js) — this
    // fires the same company-wide report the listing Print button does.
    try {
      await printReport({
        ...PRINT_REPORT_CONFIG["assets-write-off"],
        jsonParameters: buildAwfReportParams(),
      });
    } catch (err) {
  ...
  return (
    <div className="workspace-page workspace-page--fill awf-page">
      ...
      <section className="awf-grid-section" ref={itemGridSectionRef}>
        <EntryGrid
          ref={itemGridRef}
