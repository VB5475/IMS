...
import {
  ...
  PAGE_TITLE,
  PAGE_TITLE_NEW,
  buildAhsItemPickerJsonPayload,
  applyAhsHardcodedHeaderValues,
  buildAhsCascadeResets,
  validateAhsBusinessRules,
  buildAhsReportParams,
} from "./constants";
...
function resolveEditLoadParams(recordId, listRecord) {
  ...
  return {
    ...
    yearId: listRecord?.yearid ?? listRecord?.YearID ?? session.yearId ?? AHS_CONFIG.CONFIG_YEAR_ID,
...
function mapHeaderValuesToFilterValues(headerValues) {
  ...
  return {
    ...
    frmtype: str(headerValues.frmtype ?? AHS_CONFIG.FRM_TYPE),
    issuetypeid: str(headerValues.issuetypeid ?? AHS_CONFIG.ISSUE_TYPE_ID),
  };
}
...
export default function AssetsHealthStatusUpdationForm() {
  ...
  const {
    ...
    divisionOptions, toLocationOptions, toDepartmentOptions,
    configOptions,
    fetchToLocations, fetchToDepartments, fetchConfigOptions,
    ...
    clearSaveError,
  } = useAstHealthStatus(API_BASE_URL);
  ...
  const headerValuesRef = useRef(applyAhsHardcodedHeaderValues({
    trancode: "",
  ...
  const filterInitialValues = useMemo(() => {
    ...
    return {
      trandate: getTodayDateInputValue(),
      issuedate: getTodayDateInputValue(),
      frmtype: String(AHS_CONFIG.FRM_TYPE),
      issuetypeid: String(AHS_CONFIG.ISSUE_TYPE_ID),
    };
  }, [loadedFilterValues]);
  ...
  const groupFilter = useItemPickerGroupFilter({
    spMainGroup: AHS_CONFIG.SP_ITEM_MAIN_GROUP,
    spSubMainGroup: AHS_CONFIG.SP_ITEM_SUB_MAIN_GROUP,
    formTag: AHS_CONFIG.FORM_TAG,
  });
  ...
  const loadEditRecord = useCallback(async () => {
    ...
    try {
      ...
      if (!master || !headerValues) throw new Error("Assets Health Status Updation record not found.");
      ...
      console.error("[AHS] Edit record load failed:", err);
      setRecordLoadError(err?.message || "Failed to load Assets Health Status Updation record.");
    } finally {
  ...
  const dropdownSources = useMemo(() => ({
    ...
    frmtype: AHS_FRM_TYPE_OPTIONS,
  }), [divisionOptions, toLocationOptions, toDepartmentOptions, configOptions]);
  ...
  }, [requestGridClear, headerColumns, fetchConfigOptions, fetchToLocations]);
  ...
  const handleSelectItem = useCallback(async () => {
    ...
    try {
      const rbRes = await getLive(ENDPOINTS.FN_FETCH_DATA, {
        ...
        JSon: JSON.stringify([{ prmRBCode: AHS_CONFIG.RB_ITEM_PICKER }]),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      const rbRow = rbRes?.[0];
      ...
      console.error("[AHS] Item picker fetch failed:", err);
  ...
  const handleApplyItemFilter = useCallback(async () => {
    ...
    try {
      await groupFilter.applyFilter(
        async (groupParams) => {
          ...
          const rowRes = await getLive(ENDPOINTS.FN_FETCH_DATA, {
            ObjType: OBJ_TYPE.FUNCTION,
            ObjName: AHS_CONFIG.SP_ITEM_PICKER,
            JSon: JSON.stringify([{
              ...buildAhsItemPickerJsonPayload(headerValues, {
      ...
      console.error("[AHS] Item filter fetch failed:", err);
      setItemModalError(err?.message || "Failed to fetch items.");
    }
  ...
  const handleSave = useCallback(async () => {
    ...
    const hv = applyAhsHardcodedHeaderValues(headerValuesRef.current);
    ...
    const payload = await withSaveContextFields(
      buildSaveJsonFields({ label: AHS_CONFIG.FORM_TAG, mst: mstRow, det: detRows }),
  ...
  const handleSaveAndPrint = useCallback(async () => {
    ...
    if (!saved) return;
    // AssetsHealthStatusUpdation.rpt has no per-record filter (see
    // constants.js) — this fires the same company-wide report the listing
    // Print button does.
    try {
      await printReport({
        ...PRINT_REPORT_CONFIG["assets-health-status-updation"],
        jsonParameters: buildAhsReportParams(),
      });
    } catch (err) {
  ...
  ], [handleSaveAndPrint, handleSave, isSaving, isPrinting, docLog.documentsButtonEntry]);

  const itemGridConfig = { columns, pagination: { pageSize: 10, pageSizeOptions: [5, 10, 25, 50] } };
  const combinedError = metaError || headerError;

  return (
    <div className="workspace-page workspace-page--fill ahs-page">
      ...
      <section className="workspace-page__filters">
        {combinedError ? (
          ...
          <EnterpriseFilterPanel
            ...
            title="Assets Health Status Updation Detail"
          ...
          headerControls={
            <>
              ...
              <button
                ...
                disabled={!isEditMode}
                title="Pick assets for health status update"
              >
