...
export default function AssetsHealthStatusUpdationForm() {
  ...
  const fetchItemMainGroupOptions = useCallback(async ({ divisionId, configId }) => {
    try {
      ...
      const res = await getLive(ENDPOINTS.FN_FETCH_DATA, {
        ...
        ObjName: AHS_CONFIG.SP_ITEM_MAIN_GROUP,
        JSon: JSON.stringify([{
          ...
          prmfrmtype: AHS_CONFIG.FORM_TAG,
        }]),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      ...
      console.warn("[AHS] Item Main Group fetch failed:", err);
  ...
  const fetchItemSubMainGroupOptions = useCallback(async ({ divisionId, configId, mainGroupId }) => {
    ...
    try {
      ...
      const res = await getLive(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: OBJ_TYPE.FUNCTION,
        ObjName: AHS_CONFIG.SP_ITEM_SUB_MAIN_GROUP,
        JSon: JSON.stringify([{
          prmcompanyid: session.companyId,
          prmdivisionid: Number(divisionId) || 0,
          prmyearid: session.yearId,
      ...
      console.warn("[AHS] Item Sub Main Group fetch failed:", err);
      setItemSubMainGroupOptions([]);
      return [];
    }
  }, [getLive]);
