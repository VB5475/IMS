...
import { AET_CONFIG, ENTRY_FORM_LABEL, buildAetListJsonPayload, buildAetReportParams } from "./constants";
...
export default function AssetsEmployeeTransferPage() {
  ...
  const getRowState = useApprovalRowStatus("assets-employee-transfer");
  ...
  useEffect(() => {
    ...
    postWkf(ENDPOINTS.WKF_HANDLE_BUTTON_VISIBILITY, {
      prmref_is_trantypeid: AET_CONFIG.WKF_TRAN_TYPE_ID,
      ...
      .catch((err) => {
        console.warn("[AssetsEmployeeTransferPage] WKF button visibility fetch failed:", err);
        setWkfBtnVisible("NO");
      });
    ...
    get(ENDPOINTS.FN_FETCH_DATA, {
      ...
      .catch((err) => console.warn("[AssetsEmployeeTransferPage] Division options fetch failed:", err));
  }, [postWkf, get]);
  ...
  const handleSendForApproval = useCallback(async () => {
    ...
    try {
      ...
      console.error("[AssetsEmployeeTransferPage] Send for approval failed:", err);
