...
export default function CWIPToFAForm() {
  ...
  const handleSaveAndPrint = useCallback(async () => {
    ...
    if (!saved) return;
    // CWIPToFA.rpt has no per-record filter (see constants.js) — this fires
    // the same company-wide report the listing Print button does.
    try {
      await printReport({
        ...PRINT_REPORT_CONFIG["cwip-to-fa"],
        jsonParameters: buildCWIPToFAReportParams(),
      });
    } catch (err) {
