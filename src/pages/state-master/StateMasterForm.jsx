...
import { Map, Save, Pencil, AlertCircle } from "lucide-react";
...
// candidates so whichever one the live RB actually uses still resolves.
const COUNTRY_KEYS = ["countryid"];
const STATE_TYPE_KEYS = ["statetypeid", "typeid"];
...
export default function StateMasterForm({
  ...
  function resolveOptions(fieldColname) {
    ...
    if (COUNTRY_KEYS.includes(lowered)) return countryOptions;
    if (STATE_TYPE_KEYS.includes(lowered)) return stateTypeOptions;
    return [];
  }
  ...
  function buildControl(field) {
    ...
    // colctrltype 4 — Dropdown (Country, State Type)
    if (Number(field.colctrltype) === 4) {
      ...
      return (
        <SearchSelect
          ...
          placeholder="Select..."
          disabled={locked}
          className={error ? "stm-form-dropdown--error" : undefined}
        />
      );
    }
    ...
    return (
      <input
        className={`stm-form-input${error ? " stm-form-input--error" : ""}`}
        type="text"
  ...
  function renderControl(field) {
    ...
    return (
      <>
        {control}
        <div className="stm-form-field-error">{error}</div>
      </>
    );
  }
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const result = await post(STM_CONFIG.SAVE_ENDPOINT, payload);
      ...
      console.error("[STM Save] Failed:", err);
        ...
        <>
          ...
          <div className="stm-form">
            {visibleFields.map((field) => (
              <div key={field.colname} className="stm-form-row">
                <span className={`stm-form-label${field.ismandatory ? " stm-form-label--required" : ""}`}>
                  {getLabel(field)}
                </span>
                <div className="stm-form-control">
                  {renderControl(field)}
                </div>
              </div>
            ))}
