...
import {
  ...
  PAGE_TITLE,
  PAGE_TITLE_NEW,
  buildMacrItemPickerJsonPayload,
  buildMacrTermsPickerJsonPayload,
  applyMacrHardcodedHeaderValues,
  buildMacrCascadeResets,
  validateMacrBusinessRules,
} from "./constants";
import "./MaintenanceContractRenewalPage.css";

let _macrTempId = -1;
const nextTempId = () => _macrTempId--;
...
function resolveEditLoadParams(recordId, listRecord) {
  ...
  return {
    ...
    yearId: listRecord?.yearid ?? listRecord?.YearID ?? session.yearId ?? MACR_CONFIG.CONFIG_YEAR_ID,
...
function mapHeaderValuesToFilterValues(headerValues) {
  ...
  return {
    ...
    funccode: headerValues.funccode ?? MACR_CONFIG.RB_MASTER,
  };
}
...
export default function MaintenanceContractRenewalForm() {
  ...
  const {
    ...
    clearSaveError,
  } = useMntContractRenewal(API_BASE_URL);
  ...
  const headerValuesRef = useRef(applyMacrHardcodedHeaderValues({
    contractno: "",
  ...
  const loadEditRecord = useCallback(async () => {
    ...
    try {
      ...
      if (!master || !headerValues) throw new Error("Maintenance Contract Renewal record not found.");
      ...
      console.error("[MACR] Edit record load failed:", err);
      setRecordLoadError(err?.message || "Failed to load Maintenance Contract Renewal record.");
    } finally {
  ...
  const handleFilterChange = useCallback(async (colName, val) => {
    ...
    if (col === "supplierid") {
      requestGridClear("Supplier", async () => {
        itemGridRef.current?.clearRows?.();
        setItemSelectionCount(0);
      });
    }
  ...
  const handleSelectItem = useCallback(async () => {
    ...
    try {
      const rbRes = await getLive(ENDPOINTS.FN_FETCH_DATA, {
        ...
        JSon: JSON.stringify([{ prmRBCode: MACR_CONFIG.RB_ITEM_PICKER }]),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      const rbRow = rbRes?.[0];
      ...
      const rowRes = await getLive(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: OBJ_TYPE.FUNCTION,
        ObjName: MACR_CONFIG.SP_ITEM_PICKER,
        JSon: JSON.stringify([buildMacrItemPickerJsonPayload(headerValues)]),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      setItemModalItems(rowRes || []);
    } catch (err) {
      console.error("[MACR] Item picker fetch failed:", err);
  ...
  const handleSelectTerms = useCallback(async () => {
    ...
    try {
      const rbRes = await getLive(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: OBJ_TYPE.FUNCTION,
        ObjName: MACR_CONFIG.SP_RB_META,
        JSon: JSON.stringify([{ prmRBCode: MACR_CONFIG.RB_TERMS_PICKER }]),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      const rbRow = rbRes?.[0];
      ...
      const rowRes = await getLive(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: OBJ_TYPE.FUNCTION,
        ObjName: MACR_CONFIG.SP_TERMS_PICKER,
        JSon: JSON.stringify([
          buildMacrTermsPickerJsonPayload(headerValues, getUserSession().loginId),
        ]),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      ...
      console.error("[MACR] Terms picker fetch failed:", err);
  ...
  const handleSave = useCallback(async () => {
    ...
    const hv = applyMacrHardcodedHeaderValues(headerValuesRef.current);
  ...
  return (
    <div className="workspace-page workspace-page--fill macr-page">
      ...
      <section className="workspace-page__filters">
        {combinedError ? (
          ...
          <EnterpriseFilterPanel
            ...
            title="Maintenance Contract Renewal Detail"
      ...
      <section className="macr-grid-section" ref={itemGridSectionRef}>
        ...
        <div className={`macr-tab-pane${activeTab === "items" ? " macr-tab-pane--active" : ""}`}>
          <EntryGrid
            ref={itemGridRef}
        ...
        <div className={`macr-tab-pane${activeTab === "terms" ? " macr-tab-pane--active" : ""}`}>
          <EntryGrid
            ref={termsGridRef}
