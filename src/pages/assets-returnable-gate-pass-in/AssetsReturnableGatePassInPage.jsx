...
import { ARGI_CONFIG, ENTRY_FORM_LABEL, buildArgiListJsonPayload, buildGatePassReportParams } from "./constants";
...
export default function AssetsReturnableGatePassInPage() {
  ...
  const getRowState = useApprovalRowStatus("assets-returnable-gate-pass-in");
  ...
  useEffect(() => {
    ...
    postWkf(ENDPOINTS.WKF_HANDLE_BUTTON_VISIBILITY, {
      prmref_is_trantypeid: ARGI_CONFIG.WKF_TRAN_TYPE_ID,
      ...
      .catch((err) => {
        console.warn("[AssetsReturnableGatePassInPage] WKF button visibility fetch failed:", err);
        setWkfBtnVisible("NO");
      });
    ...
    get(ENDPOINTS.FN_FETCH_DATA, {
      ...
      .catch((err) => console.warn("[AssetsReturnableGatePassInPage] Division options fetch failed:", err));
  }, [postWkf, get]);
  ...
  const handleSendForApproval = useCallback(async () => {
    ...
    try {
      ...
      console.error("[AssetsReturnableGatePassInPage] Send for approval failed:", err);
