...
export default function MaintenanceNewContractPage() {
  ...
  const getRowState = useApprovalRowStatus("maintenance-new-contract");
  ...
  useEffect(() => {
    ...
    postWkf(ENDPOINTS.WKF_HANDLE_BUTTON_VISIBILITY, {
      prmref_is_trantypeid: MACNG_CONFIG.WKF_TRAN_TYPE_ID,
      ...
      .catch((err) => {
        console.warn("[MaintenanceNewContractPage] WKF button visibility fetch failed:", err);
        setWkfBtnVisible("NO");
      });
    ...
    get(ENDPOINTS.FN_FETCH_DATA, {
      ...
      .catch((err) => console.warn("[MaintenanceNewContractPage] Division options fetch failed:", err));
  }, [postWkf, get]);
  ...
  const handleSendForApproval = useCallback(async () => {
    ...
    try {
      ...
      console.error("[MaintenanceNewContractPage] Send for approval failed:", err);
