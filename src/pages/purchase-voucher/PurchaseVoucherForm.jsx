...
export default function PurchaseVoucherForm() {
  ...
  const handleSelectItem = useCallback(async () => {
    const { DivisionID, ConfigID, TranDate, BasedOnID, SupplierID } = headerValuesRef.current;
  ...
  const handleClose = useCallback(() => navigate("/purchase-voucher"), [navigate]);

  // ── Keyboard shortcuts — Alt+A | Alt+S | Alt+N | Alt+C ────────────
    ...
    const onKeyDown = (e) => {
      ...
      switch (key) {
        case "a":
          ...
          break;
        case "s":
          if (isEditMode && !isSavingPV) { e.preventDefault(); handleSave(); }
          break;
        case "n":
  ...
  }, [isEditMode, filterBusy, isSavingPV, itemModalOpen, enterEditModeWithFocus, handleSave, handleCancel, handleClose]);
  ...
  const pvExtraButtons = useMemo(() => [
    ...
    },
    {
      ...
      accessKey: "s", title: PV_SHORTCUT_CONFIG.s.title,
    },
    { key: "sep1", separator: true },
    {
      key: "close", label: "Close", Icon: LogOut, variant: "close",
      onClick: handleClose, showAlways: true,
      accessKey: "c", title: PV_SHORTCUT_CONFIG.c.title,
    },
  ], [handleSaveAndPrint, isSavingPV, handleSave, handleClose]);
