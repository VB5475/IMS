...
function buildSaveContext() {
  ...
  return {
    ...
    funccode: DMDEPT_CONFIG.RB_MASTER,
  };
}

export default function DMDepartmentMasterForm({
  isOpen,
  mode,
  onClose,
  onSaved,
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const payload = withSaveContextFields(
        buildSaveJsonFields({ label: DMDEPT_CONFIG.FORM_TAG, mst: mstRow }),
        { divisionId: 0, isEdit: !isAddMode }
      );

      const result = await post(DMDEPT_CONFIG.SAVE_ENDPOINT, payload);
      ...
      console.error("[DMDept Save] Failed:", err);
  ...
  return (
    <Modal
      ...
      subtitle="Admin › DMS › Department Master"
          ...
          <div className="dmdept-form-scroll">
            <div className="dmdept-form">
              {visibleFields.map((field) => (
                <div key={field.ColName} className="dmdept-form-row">
                  <span className={`dmdept-form-label${isMasterFieldRequired(field) ? " dmdept-form-label--required" : ""}`}>
                    {getMasterFieldLabel(field)}
                  </span>
                  <div className="dmdept-form-control">{renderControl(field)}</div>
                </div>
              ))}
            </div>
          </div>
