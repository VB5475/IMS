...
export default function AssetsEmployeeReturnForm() {
  ...
  const fetchItemMainGroupOptions = useCallback(async ({ divisionId, configId }) => {
    try {
      ...
      const res = await getLive(ENDPOINTS.FN_FETCH_DATA, {
        ...
        ObjName: AER_CONFIG.SP_ITEM_MAIN_GROUP,
        JSon: JSON.stringify([{
          ...
          prmfrmtype: AER_CONFIG.FORM_TAG,
        }]),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      ...
      console.warn("[AER] Item Main Group fetch failed:", err);
  ...
  const fetchItemSubMainGroupOptions = useCallback(async ({ divisionId, configId, mainGroupId }) => {
    ...
    try {
      ...
      const res = await getLive(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: OBJ_TYPE.FUNCTION,
        ObjName: AER_CONFIG.SP_ITEM_SUB_MAIN_GROUP,
        JSon: JSON.stringify([{
          prmcompanyid: session.companyId,
          prmdivisionid: Number(divisionId) || 0,
          prmyearid: session.yearId,
      ...
      console.warn("[AER] Item Sub Main Group fetch failed:", err);
      setItemSubMainGroupOptions([]);
      return [];
    }
  }, [getLive]);
  ...
  return (
    <div className="workspace-page workspace-page--fill aer-page">
      ...
      <section className="aer-grid-section">
        <EntryGrid
          ref={itemGridRef}
