...
export default function AssetsStockTransferForm() {
  ...
  const handleSelectItem = useCallback(async () => {
    ...
    try {
      ...
      const rowRes = await getLive(ENDPOINTS.FN_FETCH_DATA, {
        ...
        JSon: JSON.stringify([buildAstItemPickerJsonPayload(headerValues)]),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
  ...
  const handleSave = useCallback(async () => {
    ...
    setIsSaving(true);
    try {
      const res = await fetch(`${API_BASE_URL_IMS}${AST_CONFIG.SAVE_ENDPOINT}`, {
        method: "POST",
  ...
  return (
    <div className="workspace-page ast-page">
      ...
      <section className="ast-grid-section">
        ...
        <div className={`ast-tab-pane${activeTab === "items" ? " ast-tab-pane--active" : ""}`}>
          <EntryGrid
            ref={itemGridRef}
