...
import { AER_CONFIG } from "../pages/assets-employee-return/constants";
import {
...
function mapFromEmpRows(rows) {
  return (rows || []).map((r) => ({
    value: String(r.fromempuserid ?? r.userid ?? r.empuserid ?? r.idnumber ?? 0),
    label: String(
      r.fromempusername
      ?? r.fromempusercode
      ?? r.username
...
export function useAstEmpReturn(baseURL = API_BASE_URL) {
  ...
  const fetchFromDivisions = useCallback(async () => {
    try {
      const res = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: AER_CONFIG.SP_FROM_DIVISION,
      ...
      console.warn("[AER] From division fetch failed:", err);
  ...
  const fetchToLocations = useCallback(async (divisionId = 0) => {
    try {
      ...
      console.warn("[AER] To location fetch failed:", err);
  ...
  const fetchToDepartments = useCallback(async () => {
    try {
      ...
      console.warn("[AER] To department fetch failed:", err);
  ...
  const fetchConfigOptions = useCallback(
    async (divisionId = 0) => {
      ...
      try {
        ...
        console.warn("[AER] Config fetch failed:", err);
        setConfigOptions([]);
        return [];
      }
    },
  ...
  const fetchFromEmployees = useCallback(
    async (divisionId, locationId, deptId) => {
      ...
      if (!resolvedDivisionId || !resolvedLocationId || !resolvedDeptId) {
        setFromEmpOptions([]);
        return [];
      }
      try {
        const res = await get(ENDPOINTS.FN_FETCH_DATA, {
          ...
          JSon: JSON.stringify([{
            ...
            prmissuetypeid: AER_CONFIG.EMP_ISSUE_TYPE_ID,
          }]),
          p_ErrCode: -1,
          p_ErrMsg: "",
        });
        ...
        console.warn("[AER] From employee fetch failed:", err);
        setFromEmpOptions([]);
        return [];
      }
    },
  ...
  const fetchHeaderMeta = useCallback(async ({ skipListDropdowns = false } = {}) => {
    ...
    try {
      ...
      const tableRow = metaData?.[0];
      if (!tableRow) throw new Error("No AER header RB metadata returned from server.");

      const hdrMeta = { RBID: tableRow.rbid, SaveProcName: tableRow.saveprocname };
      localStorage.setItem(AER_CONFIG.STORAGE_HEADER_META, JSON.stringify(hdrMeta));
      ...
      if (hasVisibleCol(cols, "configid")) tasks.push(fetchConfigOptions(0));
      await Promise.all(tasks);
    } catch (err) {
      console.error("[AER] fetchHeaderMeta failed:", err);
      setHeaderError(err?.message || "Failed to load Assets Employee Return configuration.");
    } finally {
      setHeaderFetching(false);
    }
  }, [get, fetchFromDivisions, fetchToLocations, fetchToDepartments, fetchConfigOptions]);
  ...
  const fetchDetailMeta = useCallback(async () => {
    ...
    try {
      const { meta, apiColumns } = await loadRbDetailGridMeta(
        get,
        AER_CONFIG.RB_DETAIL,
        AER_CONFIG.STORAGE_ENTRY_META
      );
      ...
      console.error("[AER] fetchDetailMeta failed:", err);
  ...
  const fetchGridColumns = useCallback(async (divisionID = 0, editOpts = false) => {
    ...
    if (!apiColumns.length || !meta) {
      console.warn("[AER] fetchGridColumns called before fetchDetailMeta completed.");
      return [];
    }

    try {
      const colDropdownOptions = await fetchDropdownOptions(get, apiColumns, meta.RBID, {
        funcCode: AER_CONFIG.RB_DETAIL,
      ...
      console.error("[AER] fetchGridColumns failed:", err);
      return [];
    }
  }, [get]);
  ...
  const fetchUnlockedHeaderDropdowns = useCallback(
    async (headerValues = {}) => {
      ...
      if (needsCol("fromempuserid") && fromDiv) {
        tasks.push(
          fetchFromEmployees(fromDiv, headerValues.tolocationid, headerValues.todeptid)
        );
      }
    ...
    async ({ companyId, yearId, loginId, sessionId, idNumber }) => {
      ...
      const [mstRes, detRes] = await Promise.all([
        get(ENDPOINTS.GET_MASTER_DATA_FILL, {
          prmProcedure: AER_CONFIG.SP_MASTER_FILL,
          prmParameters,
          prmFuncCode: AER_CONFIG.RB_MASTER,
        }),
        get(ENDPOINTS.GET_MASTER_DATA_FILL, {
          prmProcedure: AER_CONFIG.SP_DETAIL_FILL,
          prmParameters,
          prmFuncCode: AER_CONFIG.RB_DETAIL,
        }),
      ]);
