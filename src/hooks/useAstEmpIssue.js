...
import { AEI_CONFIG } from "../pages/assets-employee-issue/constants";
import {
...
export function useAstEmpIssue(baseURL = API_BASE_URL) {
  ...
  const fetchFromDivisions = useCallback(async () => {
    try {
      const res = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: AEI_CONFIG.SP_FROM_DIVISION,
      ...
      console.warn("[AEI] From division fetch failed:", err);
  ...
  const fetchToDivisions = useCallback(async () => {
    try {
      const res = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: AEI_CONFIG.SP_TO_DIVISION,
        JSon: divisionFetchJson(),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      ...
      console.warn("[AEI] To division fetch failed:", err);
  ...
  const fetchFromLocations = useCallback(async (divisionId = 0) => {
    try {
      const res = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: AEI_CONFIG.SP_FROM_LOCATION,
      ...
      console.warn("[AEI] From location fetch failed:", err);
  ...
  const fetchToLocations = useCallback(async (divisionId = 0) => {
    try {
      const res = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: AEI_CONFIG.SP_TO_LOCATION,
        JSon: locationFetchJson(divisionId),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      ...
      console.warn("[AEI] To location fetch failed:", err);
  ...
  const fetchFromDepartments = useCallback(async () => {
    try {
      const res = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: AEI_CONFIG.SP_FROM_DEPT,
        JSon: deptFetchJson(),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      ...
      console.warn("[AEI] From department fetch failed:", err);
  ...
  const fetchToDepartments = useCallback(async () => {
    try {
      ...
      console.warn("[AEI] To department fetch failed:", err);
  ...
  const fetchFromVendors = useCallback(
    async (divisionId, locationId) => {
      try {
        ...
        console.warn("[AEI] From vendor fetch failed:", err);
  ...
  const fetchToVendors = useCallback(
    async (divisionId, locationId) => {
      try {
        ...
        console.warn("[AEI] To vendor fetch failed:", err);
        setToVendorOptions([]);
        return [];
      }
    },
  ...
  const fetchFromWorkingClients = useCallback(
    async (divisionId) => {
      try {
        ...
        console.warn("[AEI] From working client fetch failed:", err);
  ...
  const fetchToWorkingClients = useCallback(
    async (divisionId) => {
      try {
        ...
        console.warn("[AEI] To working client fetch failed:", err);
        setToClientOptions([]);
        return [];
      }
    },
  ...
  const fetchConfigOptions = useCallback(
    async (divisionId = 0) => {
      try {
        ...
        console.warn("[AEI] Config fetch failed:", err);
        setConfigOptions([]);
        return [];
      }
    },
      ...
      try {
        const res = await get(ENDPOINTS.FN_FETCH_DATA, {
          ...
          JSon: JSON.stringify([{
            ...
            prmissuetypeid: AEI_CONFIG.EMP_ISSUE_TYPE_ID,
          }]),
          p_ErrCode: -1,
          p_ErrMsg: "",
        });
        ...
        console.warn("[AEI] From employee fetch failed:", err);
        setFromEmpOptions([]);
        return [];
      }
    },
  ...
  const fetchToEmployees = useCallback(
    async (divisionId, locationId, deptId) => {
      try {
        ...
        console.warn("[AEI] To employee fetch failed:", err);
        setToEmpOptions([]);
        return [];
      }
    },
  ...
  const fetchHeaderMeta = useCallback(async ({ skipListDropdowns = false } = {}) => {
    ...
    try {
      ...
      localStorage.setItem(AEI_CONFIG.STORAGE_HEADER_META, JSON.stringify(hdrMeta));
      ...
      console.error("[AEI] fetchHeaderMeta failed:", err);
      setHeaderError(err?.message || "Failed to load Assets Employee Issue configuration.");
    } finally {
  ...
  const fetchDetailMeta = useCallback(async () => {
    ...
    try {
      const { meta, apiColumns } = await loadRbDetailGridMeta(
        get,
        AEI_CONFIG.RB_DETAIL,
        AEI_CONFIG.STORAGE_ENTRY_META
      );
      ...
      console.error("[AEI] fetchDetailMeta failed:", err);
  ...
  const fetchGridColumns = useCallback(async (divisionID = 0, editOpts = false) => {
    ...
    if (!apiColumns.length || !meta) {
      console.warn("[AEI] fetchGridColumns called before fetchDetailMeta completed.");
      return [];
    }

    try {
      const colDropdownOptions = await fetchDropdownOptions(get, apiColumns, meta.RBID, {
        funcCode: AEI_CONFIG.RB_DETAIL,
      ...
      console.error("[AEI] fetchGridColumns failed:", err);
      return [];
    }
  }, [get]);
    ...
    async ({ companyId, yearId, loginId, sessionId, idNumber }) => {
      ...
      const [mstRes, detRes] = await Promise.all([
        get(ENDPOINTS.GET_MASTER_DATA_FILL, {
          prmProcedure: AEI_CONFIG.SP_MASTER_FILL,
          prmParameters,
          prmFuncCode: AEI_CONFIG.RB_MASTER,
        }),
        get(ENDPOINTS.GET_MASTER_DATA_FILL, {
          prmProcedure: AEI_CONFIG.SP_DETAIL_FILL,
          prmParameters,
          prmFuncCode: AEI_CONFIG.RB_DETAIL,
        }),
      ]);
