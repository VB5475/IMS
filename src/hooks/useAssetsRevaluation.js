...
import { ARV_CONFIG } from "../pages/assets-revaluation/constants";
import {
...
function buildMasterDataFillParams({ companyId, yearId, loginId, sessionId, idNumber }) {
  return [
    ...
    Number(yearId) || ARV_CONFIG.CONFIG_YEAR_ID,
...
function mapMasterRowToHeaderValues(master) {
  return {
    ...
    issuedate:
      toDateInput(master.issuedate ?? master.IssueDate ?? master.RevaluationDate)
      || toDateInput(master.trandate),
...
export function useAssetsRevaluation(baseURL = API_BASE_URL) {
  ...
  const divisionFetchJson = useCallback(
    () => JSON.stringify([{
      ...
      prmyearid: ARV_CONFIG.DIVISION_YEAR_ID,
    }]),
    []
  );
  ...
  const fetchDivisions = useCallback(async () => {
    try {
      const res = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: ARV_CONFIG.SP_FROM_DIVISION,
      ...
      console.warn("[ARV] Division fetch failed:", err);
      setDivisionOptions([]);
      return [];
    }
  ...
  const fetchConfigOptions = useCallback(async (divisionId = 0) => {
    ...
    try {
      const res = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: ARV_CONFIG.SP_CONFIG,
        JSon: JSON.stringify([{
          prmcompanyid: DEFAULT_COMPANY_ID,
          prmdivisionid: resolvedDivisionId,
          prmyearid: ARV_CONFIG.CONFIG_YEAR_ID,
          prmuserid: DEFAULT_LOGIN_ID,
          prmformtag: ARV_CONFIG.CONFIG_FORM_TAG,
          prmreftype: ARV_CONFIG.CONFIG_REF_TYPE,
          prmref_mstid: 0,
          prmref_detid: 0,
        }]),
        p_ErrCode: -1,
      ...
      console.warn("[ARV] Config fetch failed:", err);
      setConfigOptions([]);
      return [];
    }
  }, [get]);
  ...
  const fetchHeaderMeta = useCallback(async ({ skipListDropdowns = false } = {}) => {
    ...
    try {
      const metaData = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: ARV_CONFIG.SP_RB_META,
        JSon: JSON.stringify([{ prmRBCode: ARV_CONFIG.RB_MASTER }]),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      const tableRow = metaData?.[0];
      if (!tableRow) throw new Error("No assets revaluation header RB metadata returned from server.");

      const hdrMeta = { RBID: tableRow.rbid, SaveProcName: tableRow.saveprocname };
      localStorage.setItem(ARV_CONFIG.STORAGE_HEADER_META, JSON.stringify(hdrMeta));
      ...
      const tasks = [];
      if (cols.some((c) => String(c.colname).toLowerCase() === "fromdivisionid" && isVisibleApiCol(c))) {
        tasks.push(fetchDivisions());
      }
      await Promise.all(tasks);
    } catch (err) {
      console.error("[ARV] header meta fetch failed:", err);
      setHeaderError(err?.message || "Failed to load Assets Revaluation header metadata.");
    } finally {
  ...
  const fetchDetailMeta = useCallback(async () => {
    ...
    try {
      ...
      setMetaError(err?.message || "Failed to load Assets Revaluation detail metadata.");
    } finally {
      setIsFetching(false);
    }
  }, [get]);
  ...
  const fetchGridColumns = useCallback(async (divisionID = 0, editOpts = false) => {
    ...
    try {
      ...
      console.error("[ARV] fetchGridColumns failed:", err);
      return [];
    }
  }, [get]);
  ...
  const fetchUnlockedHeaderDropdowns = useCallback(async (headerValues = {}) => {
    ...
    await Promise.all(tasks);
  }, [headerColumns, fetchDivisions, fetchConfigOptions]);
  ...
  const fetchEditRecord = useCallback(async ({ companyId, yearId, loginId, sessionId, idNumber }) => {
    ...
    const [mstRes, detRes] = await Promise.all([
      get(ENDPOINTS.GET_MASTER_DATA_FILL, {
        prmProcedure: ARV_CONFIG.SP_MASTER_FILL,
        prmParameters,
        prmFuncCode: ARV_CONFIG.RB_MASTER,
      }),
      get(ENDPOINTS.GET_MASTER_DATA_FILL, {
        prmProcedure: ARV_CONFIG.SP_DETAIL_FILL,
        prmParameters,
        prmFuncCode: ARV_CONFIG.RB_DETAIL,
      }),
    ]);
