...
import { AHS_CONFIG } from "../pages/assets-health-status-updation/constants";
import {
...
function buildMasterDataFillParams({ companyId, yearId, loginId, sessionId, idNumber }) {
  return [
    ...
    Number(yearId) || AHS_CONFIG.CONFIG_YEAR_ID,
...
function mapMasterRowToHeaderValues(master) {
  return {
    ...
    issuedate: toDateInput(master.issuedate ?? master.IssueDate ?? master.StatusDate) || toDateInput(master.trandate),
...
function mapLocationRows(rows) {
  return (rows || []).map((r) => ({
    value: String(r.tolocationid ?? r.ToLocationID ?? r.locationid ?? r.LocationID ?? 0),
    label: String(r.tolocation ?? r.ToLocation ?? r.locationname ?? r.LocationName ?? ""),
  }));
}

function mapDeptRows(rows) {
  return (rows || []).map((r) => ({
    value: String(r.todeptid ?? r.ToDeptID ?? r.deptid ?? r.DeptID ?? 0),
    label: String(r.todept ?? r.ToDept ?? r.todeptname ?? r.deptname ?? r.DeptName ?? ""),
  }));
}
...
export function useAstHealthStatus(baseURL = API_BASE_URL) {
  ...
  const divisionFetchJson = useCallback(
    () => JSON.stringify([{
      ...
      prmyearid: AHS_CONFIG.DIVISION_YEAR_ID,
    }]),
    []
  );
  ...
  const fetchDivisions = useCallback(async () => {
    try {
      const res = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: AHS_CONFIG.SP_FROM_DIVISION,
      ...
      console.warn("[AHS] Division fetch failed:", err);
      setDivisionOptions([]);
      return [];
    }
  ...
  const fetchToLocations = useCallback(async (divisionId = 0) => {
    try {
      ...
      console.warn("[AHS] To location fetch failed:", err);
  ...
  const fetchToDepartments = useCallback(async () => {
    try {
      ...
      console.warn("[AHS] To department fetch failed:", err);
  ...
  const fetchConfigOptions = useCallback(async (divisionId = 0) => {
    ...
    try {
      const res = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: AHS_CONFIG.SP_CONFIG,
        JSon: JSON.stringify([{
          prmcompanyid: DEFAULT_COMPANY_ID,
          prmdivisionid: resolvedDivisionId,
          prmyearid: AHS_CONFIG.CONFIG_YEAR_ID,
          prmuserid: DEFAULT_LOGIN_ID,
          prmformtag: AHS_CONFIG.CONFIG_FORM_TAG,
          prmreftype: AHS_CONFIG.CONFIG_REF_TYPE,
          prmref_mstid: 0,
          prmref_detid: 0,
        }]),
        p_ErrCode: -1,
      ...
      console.warn("[AHS] Config fetch failed:", err);
      setConfigOptions([]);
      return [];
    }
  }, [get]);
  ...
  const fetchHeaderMeta = useCallback(async ({ skipListDropdowns = false } = {}) => {
    ...
    try {
      ...
      const tasks = [];
      if (hasVisibleCol(cols, "fromdivisionid")) tasks.push(fetchDivisions());
      ...
      console.error("[AHS] fetchHeaderMeta failed:", err);
      setHeaderError(err?.message || "Failed to load Assets Health Status Updation configuration.");
    } finally {
  ...
  const fetchDetailMeta = useCallback(async () => {
    ...
    try {
      ...
      console.error("[AHS] fetchDetailMeta failed:", err);
  ...
  const fetchGridColumns = useCallback(async (divisionID = 0, editOpts = false) => {
    ...
    try {
      ...
      console.error("[AHS] fetchGridColumns failed:", err);
      return [];
    }
  }, [get]);
  ...
  const fetchUnlockedHeaderDropdowns = useCallback(async (headerValues = {}) => {
    ...
    await Promise.all(tasks);
  }, [headerColumns, fetchDivisions, fetchToLocations, fetchToDepartments, fetchConfigOptions]);
  ...
  const fetchEditRecord = useCallback(async ({ companyId, yearId, loginId, sessionId, idNumber }) => {
    ...
    const [mstRes, detRes] = await Promise.all([
      get(ENDPOINTS.GET_MASTER_DATA_FILL, {
        prmProcedure: AHS_CONFIG.SP_MASTER_FILL,
        prmParameters,
        prmFuncCode: AHS_CONFIG.RB_MASTER,
      }),
      get(ENDPOINTS.GET_MASTER_DATA_FILL, {
        prmProcedure: AHS_CONFIG.SP_DETAIL_FILL,
        prmParameters,
        prmFuncCode: AHS_CONFIG.RB_DETAIL,
      }),
    ]);
