...
export function useGridSearch(baseURL = API_BASE_URL) {
  ...
  const fetchMasterDetail = useCallback(
    async (masterID) => {
      try {
        ...
        if (detail) {
          ...
          localStorage.setItem(
            REPORT_WORKSPACE_CONFIG.STORAGE_MASTER_DETAIL,
  ...
  const handleSearch = useCallback(
    async (filterValues, filterDefs, masterID) => {
      setIsSearching(true);
      setSearchError(null);
      ...
      try {
        ...
        const paramValues = paramList.map((param) => {
          ...
          switch (paramName) {
            case "@prmCompanyID":
              return String(REPORT_WORKSPACE_CONFIG.COMPANY_ID);
            case "@prmYearID":
              return String(REPORT_WORKSPACE_CONFIG.YEAR_ID);
            case "@prmLoginID":
              return String(getUserSession().loginId);
            case "@prmSessionID":
              return String(REPORT_WORKSPACE_CONFIG.SESSION_ID);
