...
import { ACR_CONFIG } from "../pages/assets-client-release/constants";
import {
...
function buildMasterDataFillParams({ companyId, yearId, loginId, sessionId, idNumber }) {
  return [
    ...
    Number(yearId) || ACR_CONFIG.CONFIG_YEAR_ID,
...
function mapMasterRowToHeaderValues(master) {
  return {
    ...
    yearid: ACR_CONFIG.CONFIG_YEAR_ID,
...
function mapFromWorkingClientRows(rows) {
  const opts = (rows || []).map((r) => ({
    // API misspells "working" as "workig" — map those keys as-is.
    // Value must be numeric id (header + SP expect fromworkingclientid); label is display name.
    value: String(
      r.fromworkigclientid
      ?? r.fromworkingclientid
      ?? r.fromworkigclientcode
      ?? r.fromworkingclientcode
      ?? 0
    ),
    label: String(
      r.fromworkigclient
...
export function useAstCliRel(baseURL = API_BASE_URL) {
  ...
  const divisionFetchJson = useCallback(
    () => JSON.stringify([{
      ...
      prmyearid: ACR_CONFIG.DIVISION_YEAR_ID,
    }]),
    []
  );
  ...
  const locationFetchJson = useCallback(
    (divisionId = 0) => {
      ...
      return JSON.stringify([{
        ...
        prmlocationtype: "",
        prmfrmtype: String(ACR_CONFIG.FRM_TYPE),
      }]);
    },
    []
  );
  ...
  const fetchFromDivisions = useCallback(async () => {
    try {
      const res = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: ACR_CONFIG.SP_FROM_DIVISION,
      ...
      console.warn("[ACR] Division fetch failed:", err);
  ...
  const fetchToLocations = useCallback(async (divisionId = 0) => {
    try {
      const res = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: ACR_CONFIG.SP_TO_LOCATION,
      ...
      console.warn("[ACR] To location fetch failed:", err);
  ...
  const fetchToDepartments = useCallback(async () => {
    try {
      ...
      console.warn("[ACR] To department fetch failed:", err);
      setToDepartmentOptions([]);
      return [];
    }
  }, [get, deptFetchJson]);

  const fetchFromWorkingClients = useCallback(async (divisionId = 0) => {
    ...
    try {
      ...
      const res = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: ACR_CONFIG.SP_FROM_WORKING_CLIENT,
        JSon: JSON.stringify([{
          prmdivisionid: resolvedDivisionId,
          prmloginid: session.loginId || DEFAULT_LOGIN_ID,
          prmyearid: session.yearId || ACR_CONFIG.DIVISION_YEAR_ID,
          prmpartytype: ACR_CONFIG.PARTY_TYPE_CLIENT,
        }]),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      ...
      console.warn("[ACR] From working client fetch failed:", err);
      setFromClientOptions([]);
      return [];
    }
  }, [get]);
  ...
  const fetchConfigOptions = useCallback(async (divisionId = 0) => {
    ...
    try {
      const res = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: ACR_CONFIG.SP_CONFIG,
        JSon: JSON.stringify([{
          prmcompanyid: DEFAULT_COMPANY_ID,
          prmdivisionid: resolvedDivisionId,
          prmyearid: ACR_CONFIG.CONFIG_YEAR_ID,
          prmuserid: DEFAULT_LOGIN_ID,
          prmformtag: ACR_CONFIG.CONFIG_FORM_TAG,
          prmreftype: ACR_CONFIG.CONFIG_REF_TYPE,
          prmref_mstid: 0,
          prmref_detid: 0,
        }]),
        p_ErrCode: -1,
      ...
      console.warn("[ACR] Config fetch failed:", err);
      setConfigOptions([]);
      return [];
    }
  }, [get]);
  ...
  const fetchHeaderMeta = useCallback(async ({ skipListDropdowns = false } = {}) => {
    ...
    try {
      const metaData = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: ACR_CONFIG.SP_RB_META,
        JSon: JSON.stringify([{ prmRBCode: ACR_CONFIG.RB_MASTER }]),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      const tableRow = metaData?.[0];
      ...
      localStorage.setItem(ACR_CONFIG.STORAGE_HEADER_META, JSON.stringify(hdrMeta));
      ...
      console.error("[ACR] fetchHeaderMeta failed:", err);
  ...
  const fetchDetailMeta = useCallback(async () => {
    ...
    try {
      const { meta, apiColumns } = await loadRbDetailGridMeta(
        get,
        ACR_CONFIG.RB_DETAIL,
        ACR_CONFIG.STORAGE_ENTRY_META
      );
      ...
      console.error("[ACR] fetchDetailMeta failed:", err);
  ...
  const fetchGridColumns = useCallback(async (divisionID = 0, editOpts = false) => {
    ...
    try {
      ...
      console.error("[ACR] fetchGridColumns failed:", err);
      return [];
    }
  }, [get]);
  ...
  const seedOptionsFromMaster = useCallback((master) => {
    ...
    seedOne(
      master.fromworkingclientid ?? master.fromworkigclientid ?? master.fromworkigclientcode,
      master.fromworkigclient ?? master.fromworkingclient ?? master.fromworkingclientname ?? master.clientname,
      setFromClientOptions
    );
    seedOne(
  ...
  const fetchUnlockedHeaderDropdowns = useCallback(async (headerValues = {}) => {
    ...
    if (needsCol("fromworkingclientid") && fromDiv) tasks.push(fetchFromWorkingClients(fromDiv));
  ...
  const fetchEditRecord = useCallback(async ({ companyId, yearId, loginId, sessionId, idNumber }) => {
    ...
    const [mstRes, detRes] = await Promise.all([
      get(ENDPOINTS.GET_MASTER_DATA_FILL, {
        prmProcedure: ACR_CONFIG.SP_MASTER_FILL,
        prmParameters,
        prmFuncCode: ACR_CONFIG.RB_MASTER,
      }),
      get(ENDPOINTS.GET_MASTER_DATA_FILL, {
        prmProcedure: ACR_CONFIG.SP_DETAIL_FILL,
        prmParameters,
        prmFuncCode: ACR_CONFIG.RB_DETAIL,
      }),
    ]);
