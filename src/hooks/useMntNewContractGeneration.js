...
import { MACNG_CONFIG } from "../pages/maintenance-new-contract/constants";
import {
...
function buildMasterDataFillParams({ companyId, yearId, loginId, sessionId, idNumber }) {
  return [
    ...
    Number(yearId) || MACNG_CONFIG.CONFIG_YEAR_ID,
...
function mapMasterRowToHeaderValues(master) {
  const next = {
    ...master,
    yearid: getUserSession().yearId || MACNG_CONFIG.CONFIG_YEAR_ID,
...
/**
 * Maintenance Contract (New) — RB rb_mntamcnewmst
 * Header + Contract Item Detail + Terms & Condition Detail.
 */
export function useMntNewContractGeneration(baseURL = API_BASE_URL) {
  ...
  const fetchDivisions = useCallback(async () => {
    try {
      ...
      console.warn("[MACNG] Division fetch failed:", err);
      setDivisionOptions([]);
      return [];
    }
  }, [get]);
  ...
  const fetchConfigOptions = useCallback(async (divisionId = 0) => {
    ...
    try {
      ...
      const res = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: MACNG_CONFIG.SP_CONFIG,
        JSon: JSON.stringify([{
          prmcompanyid: session.companyId || DEFAULT_COMPANY_ID,
          prmdivisionid: resolvedDivisionId,
          prmyearid: session.yearId || MACNG_CONFIG.CONFIG_YEAR_ID,
          prmuserid: session.loginId || DEFAULT_LOGIN_ID,
          prmformtag: MACNG_CONFIG.CONFIG_FORM_TAG,
          prmreftype: MACNG_CONFIG.CONFIG_REF_TYPE,
          prmref_mstid: 0,
          prmref_detid: 0,
        }]),
        p_ErrCode: -1,
      ...
      console.warn("[MACNG] Config fetch failed:", err);
      setConfigOptions([]);
      return [];
    }
  }, [get]);
  ...
  const fetchSuppliers = useCallback(async (divisionId = 0) => {
    ...
    try {
      ...
      const res = await get(ENDPOINTS.FN_FETCH_DATA, {
        ...
        ObjName: MACNG_CONFIG.SP_SUPPLIER,
        JSon: JSON.stringify([{
          prmdivisionid: resolvedDivisionId,
          prmloginid: session.loginId || DEFAULT_LOGIN_ID,
          prmyearid: session.yearId || MACNG_CONFIG.CONFIG_YEAR_ID,
          prmpartytype: MACNG_CONFIG.SUPPLIER_PARTY_TYPE,
        }]),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      ...
      console.warn("[MACNG] Supplier fetch failed:", err);
      setSupplierOptions([]);
      return [];
    }
  }, [get]);
  ...
  const fetchViewOptions = useCallback(async (viewName, setter) => {
    try {
      ...
      console.warn(`[MACNG] view ${viewName} fetch failed:`, err);
      setter([]);
      return [];
    }
  }, [get]);
  ...
  const fetchContractTypes = useCallback(
    () => fetchViewOptions(MACNG_CONFIG.VIEW_CONTRACT_TYPE, setContractTypeOptions),
    [fetchViewOptions]
  );

  const fetchFrequencies = useCallback(
    () => fetchViewOptions(MACNG_CONFIG.VIEW_FREQUENCY, setFrequencyOptions),
    [fetchViewOptions]
  );
  ...
  const fetchHeaderMeta = useCallback(async ({ skipListDropdowns = false } = {}) => {
    ...
    try {
      ...
      const metaData = await get(ENDPOINTS.FN_FETCH_DATA, {
        ObjType: 2,
        ObjName: MACNG_CONFIG.SP_RB_META,
        JSon: JSON.stringify([{ prmRBCode: MACNG_CONFIG.RB_MASTER }]),
        p_ErrCode: -1,
        p_ErrMsg: "",
      });
      const tableRow = metaData?.[0];
      if (!tableRow) throw new Error("No Maintenance Contract (New) header RB metadata returned.");

      const hdrMeta = { RBID: tableRow.rbid, SaveProcName: tableRow.saveprocname };
      localStorage.setItem(MACNG_CONFIG.STORAGE_HEADER_META, JSON.stringify(hdrMeta));
      ...
      console.error("[MACNG] fetchHeaderMeta failed:", err);
      setHeaderError(err?.message || "Failed to load Maintenance Contract (New) configuration.");
    } finally {
  ...
  const fetchDetailMeta = useCallback(async () => {
    ...
    try {
      const [itemMeta, termsMeta] = await Promise.all([
        loadRbDetailGridMeta(get, MACNG_CONFIG.RB_DETAIL, MACNG_CONFIG.STORAGE_ENTRY_META),
        loadRbDetailGridMeta(get, MACNG_CONFIG.RB_TERMS_DETAIL, MACNG_CONFIG.STORAGE_TERMS_META),
      ]);
      ...
      console.error("[MACNG] fetchDetailMeta failed:", err);
  ...
  const fetchGridColumns = useCallback(async (divisionID = 0, editOpts = false) => {
    ...
    try {
      const colDropdownOptions = await fetchDropdownOptions(get, apiColumns, meta.RBID, {
        funcCode: MACNG_CONFIG.RB_DETAIL,
      ...
      console.error("[MACNG] fetchGridColumns failed:", err);
      return [];
    }
  }, [get]);
  ...
  const fetchTermsGridColumns = useCallback(async (divisionID = 0, editOpts = false) => {
    ...
    try {
      const colDropdownOptions = await fetchDropdownOptions(get, apiColumns, meta.RBID, {
        funcCode: MACNG_CONFIG.RB_TERMS_DETAIL,
        divisionID: Number(divisionID) || 0,
        existingRecordEdit,
        rowData: masterRow,
        fetchUnlockedDropdowns,
      ...
      console.error("[MACNG] fetchTermsGridColumns failed:", err);
      return [];
    }
  }, [get]);
  ...
  const fetchEditRecord = useCallback(async ({ companyId, yearId, loginId, sessionId, idNumber }) => {
    ...
    const [mstRes, detRes, termsRes] = await Promise.all([
      get(ENDPOINTS.GET_MASTER_DATA_FILL, {
        prmProcedure: MACNG_CONFIG.SP_MASTER_FILL,
        prmParameters,
        prmFuncCode: MACNG_CONFIG.RB_MASTER,
      }),
      get(ENDPOINTS.GET_MASTER_DATA_FILL, {
        prmProcedure: MACNG_CONFIG.SP_DETAIL_FILL,
        prmParameters,
        prmFuncCode: MACNG_CONFIG.RB_DETAIL,
      }),
      get(ENDPOINTS.GET_MASTER_DATA_FILL, {
        prmProcedure: MACNG_CONFIG.SP_TERMS_FILL,
        prmParameters,
        prmFuncCode: MACNG_CONFIG.RB_TERMS_DETAIL,
      }),
    ]);
