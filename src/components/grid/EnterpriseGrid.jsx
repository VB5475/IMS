...
function evaluateFilter(rowValue, operator, filterValue) {
  ...
  switch (operator) {
    ...
    case "gt":
      return !isNaN(numVal) && !isNaN(fNum) && numVal > fNum;
    case "lt":
      return !isNaN(numVal) && !isNaN(fNum) && numVal < fNum;
    case "gte":
      return !isNaN(numVal) && !isNaN(fNum) && numVal >= fNum;
...
const GridForm = forwardRef(function GridForm(
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      updateRow(rowId, updatedFields) {
        setRows((prev) =>
          prev.map((r) => (String(r.id) === String(rowId) ? { ...r, ...updatedFields } : r))
        );
      },
    }),
    [rows]
  ...
  useEffect(() => {
    setPage(1);
  }, [pageSize, columnFilters, customFilters, headerFilters, sortConfig]);
  ...
  const addCustomFilter = useCallback(() => {
    setCustomFilters((prev) => [
      ...
      {
        id: generateId(),
        column: columns.find((c) => c.filterable)?.key || "",
        operator: "eq",
        value: "",
      },
    ]);
  ...
  const renderCellControl = (row, col) => {
    ...
    switch (col.controlType) {
      case 0:
        return (
          <span className="cell-label" title={value}>
            {value}
          </span>
        );
      case 1:
        return <input type="text" {...commonProps} onBlur={makeCellBlur(row, col)} />;
      case 2: {
      ...
      {showCustomFilter && (
        <div className="custom-filter-panel">
              ...
              return (
                <div key={cf.id} className="filter-condition">
                  ...
                  <select
                    ...
                    value={cf.column}
                    onChange={(e) => updateCustomFilter(cf.id, "column", e.target.value)}
                  >
                    {columns
                  ...
                  <select
                    className="op-select"
                    value={cf.operator}
                    onChange={(e) => updateCustomFilter(cf.id, "operator", e.target.value)}
                  >
                    {ops.map((op) => (
          ...
          <div className="filter-actions">
            ...
            <button className="primary" onClick={applyCustomFilters}>
              Apply Filters
            </button>
            <button
              onClick={() => {
                            ...
                            <div className="date-field">
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  setColumnFilters((prev) => ({
                                    ...prev,
                                    [col.key]: { ...current, from: e.target.value },
                                  }));
                                }}
                              />
                            </div>
                            <div className="date-field">
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  setColumnFilters((prev) => ({
                                    ...prev,
                                    [col.key]: { ...current, to: e.target.value },
                                  }));
                                }}
                              />
                            </div>
                          ...
                          <>
                            ...
                            <div className="popup-list">
                              {(() => {
                                ...
                                const allSelected =
                                  filtered.length > 0 && filtered.every((v) => currentSet.has(v));
                                return (
                                  <>
                                    <div
                                      ...
                                      onClick={() => {
                                        ...
                                        if (allSelected) {
                                          filtered.forEach((v) => newSet.delete(v));
                                        } else {
                                          filtered.forEach((v) => newSet.add(v));
                                        }
                                    ...
                                    {filtered.map((val) => (
                                      ...
                                      >
                                        <input
                                          ...
                                          readOnly
                                        />
                                        <label title={getDropdownLabel(col, val)}>
                                          {getDropdownLabel(col, val)}
                                        </label>
                                      </div>
                                    ))}
                                    {filtered.length === 0 && (
                  ...
                  >
                    {col.key === "cb" ? (
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setHeaderFilters((prev) => ({ ...prev, [col.key]: e.target.value }))
                        }
                        onKeyDown={(e) => {
                          if (e.key === "Escape")
