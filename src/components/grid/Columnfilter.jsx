...
export default function ColumnFilter({
  ...
  const renderList = () => {
    ...
    const filtered = search
      ? all.filter((v) => resolveLabel(v).toLowerCase().includes(search.toLowerCase()))
      : all;
    ...
    return (
      <>
        ...
        <div className="cf-list">
          ...
          {filtered.length === 0 && <div className="cf-empty">No values found</div>}
        </div>
      </>
    );
  };
  ...
  const popup = (
    ...
    >
      <div className="cf-header">
        <span className="cf-header-icon" aria-hidden="true">
          {ICONS[filterType]}
        </span>
      ...
      <div className="cf-footer">
        ...
        <button className="cf-btn cf-btn--primary" onClick={onClose}>
          Close
        </button>
      </div>
    </div>
